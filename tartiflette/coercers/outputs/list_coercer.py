...
__all__ = (
    ...
    "list_coercer_sequentially_serial",
...
@null_coercer_wrapper
async def list_coercer_sequentially_serial(
    result: Any,
    info: "ResolveInfo",
    execution_context: "ExecutionContext",
    field_nodes: List["FieldNode"],
    ...
    """
    Serial version of list_coercer_sequentially that uses serial execution
    for nested values (ensures complete_value_catching_error_serial is used).
    :param result: resolved value
    :param info: information related to the execution and the resolved field
    :param execution_context: instance of the query execution context
    :param field_nodes: AST nodes related to the resolved field
    ...
    for index, item in enumerate(result):
        try:
            # Use serial version for nested value completion
            # We need to get the output_coercer for the item type with serial mode
            from tartiflette.coercers.outputs.compute import get_output_coercer
            
            item_output_coercer = get_output_coercer(item_type, concurrently=False)
            value = await complete_value_catching_error_serial(
                item,
                info,
                execution_context,
                field_nodes,
